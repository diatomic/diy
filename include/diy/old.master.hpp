  ...
  class Master
  {
    ...
    public:
      ...
      */
      ...
      inline void   destroy(int i)
      {
        ...
        if (block_info_access->blocks_.own())
      ...
      const Collection&
                    blocks() const                      { return block_info_.const_access()->blocks_; }

      //! return the `i`-th block, loading it if necessary
      void*         get(int i)                          { return block_info_.access()->blocks_.get(i); }
      template<class Block>
      Block*        get(int i)                          { return static_cast<Block*>(get(i)); }

      inline Link*  link(int i) const                   { return block_info_.const_access()->links_[i]; }

      inline int    loaded_block() const                { return block_info_.const_access()->blocks_.available(); }
      ...
      bool          local(int gid__) const              { auto block_info_access = block_info_.const_access(); return block_info_access->lids_.find(gid__) != block_info_access->lids_.end(); }
      ...
      int           in_memory() const                   { return *block_info_.const_access()->blocks_.in_memory().const_access(); }
      ...
      CreateBlock   creator() const                     { return block_info_.const_access()->blocks_.creator(); }
      DestroyBlock  destroyer() const                   { return block_info_.const_access()->blocks_.destroyer(); }
      LoadBlock     loader() const                      { return block_info_.const_access()->blocks_.loader(); }
      SaveBlock     saver() const                       { return block_info_.const_access()->blocks_.saver(); }
    ...
    public:
      ...
      void              replace_link(int i, Link* link__)
      {
          ...
          delete block_info_access->links_[i];
          block_info_access->links_[i] = link__;
          expected_ += block_info_access->links_[i]->size_unique();
      }
    ...
    private:
      ...
      critical_resource<struct BlockInfo, recursive_mutex> block_info_;
...
{
    auto block_info_access = block_info_.access();
    block_info_access->blocks_.set_create(create_);
    block_info_access->blocks_.set_destroy(destroy_);
    block_info_access->blocks_.set_storage(storage);
    block_info_access->blocks_.set_save(save);
    block_info_access->blocks_.set_load(load_);
...
clear()
{
  ...
  for (unsigned i = 0; i < size(); ++i)
    delete block_info_access->links_[i];
  block_info_access->blocks_.clear();
  block_info_access->links_.clear();
  block_info_access->gids_.clear();
  block_info_access->lids_.clear();
  expected_ = 0;
}
...
unload(int i)
{
  ...
  block_info_.access()->blocks_.unload(i);
  unload_queues(i);
}
...
load(int i)
{
  ...
  block_info_.access()->blocks_.load(i);
  load_queues(i);
}
...
add(int gid__, void* b, Link* l)
{
  ...
  block_info_access->blocks_.add(b);
  block_info_access->links_.push_back(l);
  block_info_access->gids_.push_back(gid__);

  int lid__ = static_cast<int>(block_info_access->gids_.size()) - 1;
  block_info_access->lids_[gid__] = lid__;
